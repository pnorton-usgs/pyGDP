...
def point_to_geoserver(step):
    ...
    WFS_URL = 'https://cida.usgs.gov/gdp/geoserver/wfs'
...
def ask_and_you_shall_recieve(step):
    test_pyGDP = create_web_processing_object()
    world.feature_collection = test_pyGDP._getFeatureCollectionGeoType(world.shapefile, world.attribute, world.value, 'https://cida.usgs.gov/gdp/geoserver/wfs')
...
def every_of_the_conus(step):
    test_pyGDP = create_web_processing_object()
    value = None
    world.feature_collection = test_pyGDP._getFeatureCollectionGeoType(world.shapefile, world.attribute, value, 'https://cida.usgs.gov/gdp/geoserver/wfs')
