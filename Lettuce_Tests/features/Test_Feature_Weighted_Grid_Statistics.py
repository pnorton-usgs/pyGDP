...
def feature_weighted_grid_statistics(step):
    ...
    world.output_file = test_pyGDP.submitFeatureWeightedGridStatistics(world.shapefile,
                                                                       world.dataset_uri, world.data_type,
                                                                       world.time_start, world.time_end,
                                                                       world.attribute, world.value, verbose=False,
                                                                       outputfname='testFWGS_testfile_test',
                                                                       sleepSecs=5)
...
def multi_feature_weighted_grid_statistics(step):
    ...
    world.output_file = test_pyGDP.submitFeatureWeightedGridStatistics(world.shapefile, world.dataset_uri,
                                                                       ...
                                                                       gmlIDs, verbose, coverage,
                                                                       delim, world.stats,
                                                                       outputfname='testFWGS_testfile_multitest',
                                                                       sleepSecs=5)
...
def full_feature_weighted_grid_statistics(step):
    test_pyGDP = create_web_processing_object()
    outputFile_handle = test_pyGDP.submitFeatureWeightedGridStatistics(geoType=world.shapefile,
                                                                       dataSetURI=world.dataset_uri,
                                                                       varID=world.data_type,
                                                                       startTime=world.time_start,
                                                                       endTime=world.time_end,
                                                                       attribute=world.attribute,
                                                                       value=world.value, gmlIDs=None, verbose=False,
                                                                       coverage='true', delim='COMMA',
                                                                       stat=world.stats, grpby='STATISTIC',
                                                                       timeStep='false', summAttr='false',
                                                                       outputfname='testFWGS_testfile_vartest')
...
def timeless_FWGS(step):
    ...
    time_end = None
    test_pyGDP = create_web_processing_object()
    world.output_file = test_pyGDP.submitFeatureWeightedGridStatistics(geoType=world.shapefile,
                                                                       dataSetURI=world.dataset_uri,
                                                                       varID=world.data_type, startTime=time_start,
                                                                       endTime=time_end, attribute=world.attribute,
                                                                       value=world.value, gmlIDs=None, verbose=False,
                                                                       coverage='true', delim='COMMA', stat=world.stats,
                                                                       grpby='STATISTIC', timeStep='false',
                                                                       summAttr='false')
